...
def solve_13_14(graph, houses, infra, speed=40):
    ...
    for obj in infra:
        routes = lib.getFormSingleToManyPaths(graph, obj, houses, speed)
...
def solve_21(graph, houses, infra, speed=40):
    routes = lib.getFormSingleToManyPaths(graph, infra, houses, speed)
...
def solve_22_24(graph, houses, infra, speed=40):
    ...
    for k in [2, 3, 5]:
        ...
        for c in range(k):
            ...
            # 2.3 c,d)
            routes = lib.getFormSingleToManyPaths(graph, centroids[-1], cluster, speed)
            paths_length = sum(route['length'] for route in routes.values())
            routes = [route['route'] for route in routes.values()]
            tree = lib.routes_to_tree(graph, routes)
            trees.append(tree)
        ...
        # 2.3 b)
        routes = lib.getFormSingleToManyPaths(graph, infra, centroids, speed)
        paths_length = sum(route['length'] for route in routes.values())
        routes = [route['route'] for route in routes.values()]
        tree = lib.routes_to_tree(graph, routes)
        solution[k] = {
