    ...
    get: ({ get }) => {
        ...
        for (let nodeId in graph) {
            if (!graph[nodeId].hasOwnProperty('tag')) {
                ...
                if (selectedHouses.includes(nodeId) || (get(selectedInfraState).includes(nodeId))) {
...
function getPathBetween(startId, endId, pathType) {
    const route = pathType === 'round' ? dist['paths'][`${startId}-${endId}`]['route'] : dist['paths'][startId][endId]['route'];
